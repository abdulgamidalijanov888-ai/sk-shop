...
export default function ProfilePage() {
  ...
  const handleLogout = () => {
    localStorage.removeItem('sk-shop-current-user');
    localStorage.removeItem('sk-shop-token');
    setCurrentUser(null);
  };
    ...
    <div style={{ minHeight: '100vh', backgroundColor: darkMode ? '#0A0A0A' : '#FFFFFF', paddingBottom: '100px' }}>
      <div style={{ maxWidth: '600px', margin: '0 auto', padding: '16px' }}>
        ...
        <div style={{ backgroundColor: darkMode ? '#1C1C1E' : '#FFFFFF', borderRadius: '20px', padding: '24px', marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '16px', boxShadow: darkMode ? '0 4px 16px rgba(0,0,0,0.3)' : '0 4px 16px rgba(0,0,0,0.06)' }}>
          ...
          <div>
            <h2 style={{ fontSize: '20px', fontWeight: '700', marginBottom: '4px', color: darkMode ? '#FFFFFF' : '#000000' }}>
              {currentUser ? currentUser.name : 'Гость'}
            </h2>
            <p style={{ color: '#8E8E93' }}>
              {currentUser ? currentUser.email : 'Войдите или зарегистрируйтесь'}
            </p>
          </div>
        </div>
            ...
            <>
              <Link href="/auth/signin" style={{ textDecoration: 'none' }}>
                <div style={{
                  ...
                  <span style={{ color: darkMode ? '#FFFFFF' : '#000000', fontWeight: '600' }}>Войти</span>
                </div>
              </Link>
              <Link href="/auth/register" style={{ textDecoration: 'none' }}>
                <div style={{
                  ...
                  <span style={{ color: '#000', fontWeight: '600' }}>Зарегистрироваться</span>
                </div>
              </Link>
            </>
          )}
